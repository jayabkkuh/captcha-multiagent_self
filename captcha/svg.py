...
def sequence_chars_grid_svg(chars: str = "验证码", rows: int = 2, cols: int = 4, width: int = 280, height: int = 140):
    ...
    # Add cell index numbers for visual aid (semi-transparent)
    for r in range(rows):
        for c in range(cols):
            idx = r * cols + c
            # Place number in top-left corner of cell
            num_x = c * cell_w + 10
            num_y = r * cell_h + 20
            num_size = min(cell_w, cell_h) * 0.28  # Increased from 0.2
            parts.append(f"<text x='{num_x:.1f}' y='{num_y:.1f}' font-size='{num_size:.1f}' fill='#888' fill-opacity='0.7' font-family='Arial,sans-serif' font-weight='bold'>{idx}</text>")
...
def add_axis(svg: str, width: int, height: int) -> str:
    """Inject coordinate axes into an existing SVG string."""
    if "</svg>" not in svg:
        return svg
    
    # Add a group with axes
    # X axis: Red, Y axis: Green
    # Origin is top-left (0,0) in SVG, but usually we want to visualize it clearly.
    # The user prompt mentions "add coordinate axes to the image".
    # We'll draw lines along the left and top edges, and maybe a grid or ticks.
    
    axis_svg = f"""
    <g pointer-events="none" opacity="0.7">
        <!-- X Axis -->
        <line x1="0" y1="0" x2="{width}" y2="0" stroke="red" stroke-width="4"/>
        <text x="{width-20}" y="15" fill="red" font-weight="bold" font-size="14">X</text>
        
        <!-- Y Axis -->
        <line x1="0" y1="0" x2="0" y2="{height}" stroke="green" stroke-width="4"/>
        <text x="5" y="{height-10}" fill="green" font-weight="bold" font-size="14">Y</text>
