...
7) Consistency rule: once you have produced a valid English word using all given letters and it fits the prompt, do NOT switch to a different word unless you find a clear error in your previous answer.
...
"""},
    ...
    def _solve_click(self, captcha: CaptchaData, image_uri: Optional[str] = None) -> AgentResult:
        ...
        # Prefer client-provided PNG (captures front-end view). Otherwise, embed axes into SVG.
        if image_uri:
            ...
            axis_instruction = f"Coordinate System: Origin (0,0) is Bottom-Left. X axis is Red (Right). Y axis is Green (Up). Image size: {width} x {height}."
...
"""},
        ...
        for i in range(max_retries):
            ...
            try:
                ...
                # Convert from bottom-left origin (LLM view) to SVG/top-left origin for verification/execution
                raw_y = y
                y = height - 1 - raw_y
                ans = f"{x},{y}"
            except:
